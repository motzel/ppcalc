<script>
	...
	$: modifiedTotalPlayerPp = scores?.length
		? getTotalPlayerPp(
				scores,
				Object.entries(modifiedScores).reduce((cum, [leaderboardId, modified]) => {
					...
					const pp = getPpFromAccAndStars(
						modified.percentage,
						modified.stars,
						playerData?.service ?? 'scoresaber',
						ratings,
						leaderboard?.difficulty?.modeName ?? 'Standard'
					);
...
<main>
	{#if playerInfo}
		...
		<div class="box has-shadow">
			<Scores
				{scores}
				service={playerData?.service ?? 'scoresaber'}
				{modifiedScores}
...
<style>
	...
	.playlist {
		...
		padding: 0.5rem;
		top: 0.5rem;
		right: calc((100% - min(100%, 1150px + 2rem)) / 2 + 0.5rem);
