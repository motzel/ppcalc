<script>
	...
	$: secondaryMetricsType =
		secondaryMetricsPref === 'attribution' && attribution !== null && attribution !== undefined ? 'attribution' : 'weighted';
	$: secondaryMetrics = secondaryMetricsType === 'attribution' ? attribution : weighted;
	$: secondaryMetricsTitle =
		secondaryMetricsType === 'attribution'
			? `Actual contribution of the score to the total ${suffix.toUpperCase()}`
			: `Weighted ${suffix.toUpperCase()}`;
</script>
...
<span class="pp" style="--color: {color}">
	...
	<span class="value">
		<Value
			...
			{zero}
			{withZeroSuffix}
			prevValue={secondaryMetrics}
			prevWithSign={secondaryMetricsType === 'attribution'}
			prevTitle={secondaryMetricsTitle}
			prevAbsolute={secondaryMetrics !== null}
			{suffix}
			{...$$restProps}
