<script>
	...
	import {accFromPpFactor, calcPpBoundary, getPpFromAccAndStars, getWhatIfScore, PP_PER_STAR, ppFactorFromAcc} from '../utils/pp';
	...
	function getStarsForAcc(rawPp, acc) {
		return service === 'beatleader' ? (rawPp / PP_PER_STAR / acc) * 100 : rawPp / PP_PER_STAR / ppFactorFromAcc(acc);
	}

	function getAccForStars(rawPp, stars) {
		return service === 'beatleader' ? (rawPp / PP_PER_STAR / stars) * 100 : accFromPpFactor(rawPp / PP_PER_STAR / stars);
	}
	...
	async function resetCalc(scores) {
		...
		const whatIf = getWhatIfScore(scores, -1, getPpFromAccAndStars(100, maxStars, service));
			...
			<div>
				<div>
					...
					<div>
						...
						<div class="range">
							<input type="range" min="0.1" max={maxStars} step="0.01" bind:value={stars} on:input={onStarsPercentChange} />
